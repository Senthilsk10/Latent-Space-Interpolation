...
def load_image(t1, t2, bbox):
    base_url = ("https://mosdac.gov.in/live_data/wms/live3RL1BSTD1km/products/Insat3r/3R_IMG/2024/11DEC/3RIMG_11DEC2024_{timestamp}_L1B_STD_V01R00.h5?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetMap&FORMAT=image%2Fpng&TRANSPARENT=true&LAYERS=IMG_VIS&COLORSCALERANGE=46%2C538&BELOWMINCOLOR=extend&ABOVEMAXCOLOR=extend&transparent=true&format=image%2Fpng&STYLES=boxfill%2Fgreyscale&WIDTH=256&HEIGHT=256&CRS=EPSG%3A3857&BBOX={x_min},{y_min},{x_max},{y_max}")
    
    images = []
    for timestamp in [t1, t2]:
        url = base_url.format(timestamp=timestamp, x_min=bbox[0], y_min=bbox[1], x_max=bbox[2], y_max=bbox[3])
        try:
            img = get_image(url)
            if img is not None:
                images.append(img)
